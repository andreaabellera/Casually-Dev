<script>
    ...
    export let title = "Unidentified drink";
...
<div class="cover-ctr" role="group" aria-label="Drink review display for {title}">
    <div class="cover" on:mouseenter={expand} on:mouseleave={shrink} on:focus={expand} on:blur={shrink}>
        <div class="cover-inner">
            {#if image}
            <div class="revueThumb" style="background-image:url(https://4everland.io/ipfs/{image});"></div>
            {:else}
            <div></div>
            {/if}
            <div class="revueContent">
                <div class="revueTitle"> {title} </div>
                <div class="revueDetail"> {detail} </div>
                <p class="revueBlurb"> {@html blurb} </p>
            </div>
            <div id="rating"> <span id="num">{rating}</span>/10 </div>
        </div>
    </div>
</div>
